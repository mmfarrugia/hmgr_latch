#!/afs/crc.nd.edu/user/m/mfarrugi/m/bin/python
...
#            Please change them to the "normal" default values before a real run. This is a test run on the HMGR TS2 system after stripping it and making all residues standardized ones.
###################################################################################################################################################################

rosetta_scripts_path = os.path.expanduser("/opt/crc/r/rosetta/2021.38/rosetta.source.release-296/main/source/bin/rosetta_scripts.default.linuxgccrelease")
nstruct = 100 # 100 for better averaging, normally 35
max_minimization_iter = 50000 # Normally 5000, pumped up to 50,000 to be exhaustive
abs_score_convergence_thresh = 1.0 # Normally 1.0
number_backrub_trials = 50000 # Normally 35000 but 50,000 for exhaustiveness
backrub_trajectory_stride = 50000 # Can be whatever you want, if you would like to see results from earlier time points in the backrub trajectory. 7000 is a reasonable number, to give you three checkpoints for a 35000 step run, but you could also set it to = number_backrub_trials for quickest run time (as the final minimization and packing steps will only need to be run one time).
path_to_script = 'ddG-backrub.xml'
residue_to_mutate = (str(sys.argv[1]), int(sys.argv[2]), '') # Residue position to perfrom saturation mutatagenesis. Format: (Chain, PDB residue number, insertion code).
